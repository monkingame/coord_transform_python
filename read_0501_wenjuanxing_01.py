...
# path_lv=r'C:\Users\sun\OneDrive\数字淄博\开发-淄博烧烤\20230502-数据\截止到20230502-问卷星导入记录-100.xlsx'
# df_lv = pd.read_excel(path_lv, sheet_name='20230501-吕金声',
#                            converters={r'1、“一店一码”商户编码':str,r'4、统一社会信用代码':str})

# for index,row in df_lv.iterrows():
#     credit=row[r'统一社会信用代码']
#     # code=row[r'lat']
#     # idNo=row[r'7、法定代表人身份证号']
#     # print(index)
...
#         # df_wenjuan.at[index, '一店一码'] = uniqueid
#         # print(code,idNo)
#         code = dict.get(credit)
#         if code is not None:
#             # print(code)
#             # df_lv['lat']=code
#             df_lv.at[index, 'lat'] = code
#             # print(credit,code)

# df_lv.to_excel(path_lv, sheet_name='20230501-吕金声',index=False)

# print(dict)


all_wenjuanxing=r'C:\Users\sun\OneDrive\数字淄博\开发-淄博烧烤\20230502-数据\截止到20230502-问卷星导入记录-1.xlsx'
df_wenjuan = pd.read_excel(all_wenjuanxing, sheet_name='20230501-吕金声-新整理',
                           converters={'唯一编码':str,'lat':str})
...
for index,row in df_table.iterrows():
    ...
    if not pd.isna(id) and not pd.isna(uniqueid):
        ...
        df_table.at[index, '数据来源'] = '问卷星'
        df_table.at[index, '一店一码'] = uniqueid
        df_table.at[index, '数据版本'] = '501'
        # print(id,uniqueid,name)
        # print(id,uniqueid,name)
